    ...
    if(req.path != "/login" ) {
        if(req.session.loged === undefined){
          return  res.render("notFound.ejs")
            ...
            await bcrypt.compare(process.env.ADMIN_SESSION_PASSWORD,req.session.loged).then(result=>{
                if(!result) return error = "A error what to do. Call Kanishk Ranjan dog!";   
            });
        }
    }
...
router.get('/view',async (req,res)=>{
    ...
    await product.find().sort({"date":-1}).skip(parseInt(till)).limit(20).then(doc =>{
...
router.get('/view/:name',async (req,res)=>{
    ...
    max = Math.max.apply(null, result.productLinks.map(item => {
        return item.cost;
    }));

    min = Math.min.apply(null, result.productLinks.map(item => {    
        return item.cost;
    }));
    ...
    if(result.length == 0) {
        ...
        res.render('admin/adminProductView.ejs', {productInformation:result,
                                            cost:{max,min},
                                            recommendeds: []
        });
    } 
...
router.get('/logout',(req,res)=>{
    req.session.destroy(function(err) {
        if( err ) return res.send("<a href='/'>Go home</a><br><h1>Error: "+ err+"<h2>");
        res.send("<a href='/'>Go home</a><br><h1>Worked done successful</h2>");
    })
});
router.get('/edit/:name', async (req,res)=>{
    const result = await product.findOne({name:req.params.name}).sort({"date":-1});
    res.render('admin/editProduct.ejs',{values:result});
})

router.post("/login",async(req,res)=>{
    await bcrypt.compare(req.body.password,process.env.ADMIN_PASSWORD, (err, result)=>{
        if(!result) return res.redirect("/");
        req.session.loged = "$2b$10$GDlw3MuJlHVjZi1VSs0NS.d/yhzGp3PKWUNEg5B9HscwjCydmnl5a";
        res.redirect("/admin");
    })
})
...
router.post('/view',async (req,res)=>{
    till = req.query.skip || 0;
    await product.find({name:req.body.name}).sort({"date":-1}).skip(parseInt(till)).limit(20).then(doc =>{
        res.render('admin/adminView.ejs',{products:doc})
    })
});
...
router.post('/',async (req,res)=>{
    ...
    product.find({ name: req.body.name}).sort({"date":-1}).then(doc=>{
        if(doc.length !== 0){ 
            // send if data exgist
        res.render('admin/adminPanel.ejs',{msg:'A product with same name already exgist'})
        } else {    
            productData.save();
...
router.post("/view", async (req,res)=>{
    const result = await product.find({name: req.body.name}).sort({"data":-1}).limit(20);
    const recommendeds = await product.find({searchKeyWord:{ $in :req.body.name}}).sort({"data":-1}).limit(20);
